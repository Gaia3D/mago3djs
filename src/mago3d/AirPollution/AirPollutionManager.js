 ...
 */
var AirPollutionManager = function (options) 
{
	if (!(this instanceof AirPollutionManager)) 
	{
		throw new Error(Messages.CONSTRUCT_ERROR);
	}

	this.magoManager;
	this.airPollutionLayersArray;
...
AirPollutionManager.prototype.createDefaultShaders = function ()
{
...
AirPollutionManager.prototype._preparePollutionGeoJsonIndexFile = function ()
{
	if (this._geoJsonIndexFileLoadState === CODE.fileLoadState.READY)
	{
...
AirPollutionManager.prototype.newAirPollutionLayer = function (options)
{
	if (!this.airPollutionLayersArray)
	{
		this.airPollutionLayersArray = [];
	}
	...
	options.airPollutionManager = this;
	var airPollLayer = new AirPollutionLayer(options);
	this.airPollutionLayersArray.push(airPollLayer);
	return airPollLayer;
};

AirPollutionManager.prototype.prepareVolume = function (magoManager)
{
	// We need:
	// 1- GeoJsonIndexFile.
...
AirPollutionManager.prototype._preparePollutionLayers = function (magoManager)
{
	...
	if (this.airPollutionLayersArray === undefined)
	{
		this.airPollutionLayersArray = [];
	}
	...
	if (pollutionLayersCount === 0)
	{
		...
		for (var i=0; i<layersCount; i++) // usually layersCount = 1.***
		{
			...
			{
				if (airPollLayer._timeSlicesArray === undefined)
				{
					airPollLayer._timeSlicesArray = [];
				}
