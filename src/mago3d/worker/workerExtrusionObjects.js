...
worker.onmessage = function (e) 
{
	...
	for (var i=0; i<objectsCount; i++) 
	{
		...
		var divideLevel = object.divideLevel;
		...
		for (var j=0; j<geoCoordsListsCount; j++)
		{
			...
			while (accum < height) 
			{
				...
				if (divideLevel)
				{
					surfIndepMesh.material = new Material_('test');
					surfIndepMesh.material.setDiffuseTextureUrl(imageUrl);

					surfIndepMesh.calculateTexCoordsByHeight(floorHeight);
