...
{          \n\
    ...
    {    	 \n\
        //vec3 sample = origin + (tbn * kernel[i]) * radius;\n\
		vec3 sample = origin + (kernel[i]*rvec) * radius;\n\
	...
	if(occlusion <= 0.0)\n\
		discard;\n\
    occlusion = 1.0 - occlusion / (0.5*float(kernelSize));\n\
\n\
...
\n\
    ...
    {	\n\
		...
		gl_Position = pos4;\n\
		vec3 screenCoord = gl_Position.xyz;\n\
		screenCoord /= gl_Position.w;\n\
		screenCoord.x = (screenCoord.x+1.)*screenWidth/2.; \n\
		screenCoord.y = (screenCoord.y+1.)*screenHeight/2.; \n\
		vec2 screenPos = vec2(screenCoord.x, screenCoord.y);	\n\
		//vec2 screenPos = vec2(screenCoord.x*screenWidth, screenCoord.y*screenHeight);	\n\
		float linearDepth = getDepth(screenPos);\n\
		vec3 origin = getViewRay(screenPos) * linearDepth;   \n\
		gl_Position.z = -linearDepth/256.0;\n\
		//gl_Position.z = origin.z;\n\
	}\n\
";
