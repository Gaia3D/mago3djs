...
/**
 * @class ElectroMagnetismLayer
 */
var ElectroMagnetismLayer = function (options) 
{
	if (!(this instanceof ElectroMagnetismLayer)) 
	{
		throw new Error(Messages.CONSTRUCT_ERROR);
	}
     
	this._electroMagnetismManagerOwner;
	...
	{
		// check for options.***
		if (options.electroMagnetismManagerOwner !== undefined)
		{
			this._electroMagnetismManagerOwner = options.electroMagnetismManagerOwner;
		}
...
ElectroMagnetismLayer.prototype._prepareLayer = function ()
{
	if (this._isPrepared)
	{
		return true;
	...
	if (this._timeSlicesArray.length === 0)
	{
		...
		for (var i=0; i<timeSliceFileNamesCount; i++)
		{
			...
			var options = {
				filePath            : filePath,
				EMSurfaceLayerOwner : this
			};
			var timeSlice = new ElectroMagnetismTimeSlice(options);
	...
	if (this._makingMeshProcess === CODE.processState.NO_STARTED)
	{
		...
		var magoManager = this._electroMagnetismManagerOwner.magoManager;
...
ElectroMagnetismLayer.prototype.render = function (magoManager)
{
	...
	var animationStartTime = this._electroMagnetismManagerOwner._animationStartTime;
	...
	var currentShader = shaderManager.getShader("electroMagnetismSurface"); // ElectroMagnetismSurfaceVS ElectroMagnetismSurfaceFS.***
	...
	gl.uniform1i(currentShader.colorType_loc, this._electroMagnetismManagerOwner._colorType); 

	// Color legend.***
	var legendColors = this._electroMagnetismManagerOwner._legendColors4;
	var legendValues = this._electroMagnetismManagerOwner._legendValues;
