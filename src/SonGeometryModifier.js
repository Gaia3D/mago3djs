...
//		t.z= q.x*m[0][2] + q.y*m[1][2] + q.z*m[2][2] + m[3][2];
	...
	*/
	...
	// SplitValues.*************************************************************************************************************************
	f4d_splitValue = function()
	{
		// Idea from Cesium.***
		// This is used to save a Float64 in to a 2 Float32.***
		this.high = undefined;
		this.low = undefined;
	};
	
	f4d_geometryModifier.prototype.Calculate_splitValues = function(value, result)
	{
		// Code copied from Cesium's "EncodedCartesian3.encode = function(value, result)".********
		//if (!defined(value)) {
